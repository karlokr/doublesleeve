...
// ---------------------------------------------------------------------------
// 1. stock backs - a second product image for every serialised CARD
// ---------------------------------------------------------------------------
/**
 * Cards only, established by card_identity rather than by being serialised.
 *
 * Serialisation used to mean "is a card", so this selected straight off
 * card_copy. It does not any more: sealed product carries serials too now, and
 * on that reading every booster pack and box was given a photograph of the back
 * of a Pokémon card as its second gallery image - an image of something that is
 * not the product, on a listing whose whole point is that it is still sealed.
 */
$singles = $db->executeS(
    'SELECT DISTINCT cc.id_product, pl.link_rewrite
       FROM ' . _DB_PREFIX_ . 'card_copy cc
       JOIN ' . _DB_PREFIX_ . 'card_identity ci ON ci.id_product = cc.id_product
...
/**
 * Take back the ones that should never have been added.
 *
 * A guard that only stops NEW mistakes leaves the existing ones on the
 * storefront for ever, and this pass is the only thing that knows which images
 * are card backs. Identified by the legend it writes above, and removed only
 * from products that are not cards.
 */
$strays = $db->executeS(
    'SELECT i.id_image FROM ' . _DB_PREFIX_ . 'image i
       JOIN ' . _DB_PREFIX_ . 'image_lang il
            ON il.id_image = i.id_image AND il.id_lang = ' . $defaultLang . '
      WHERE il.legend LIKE "%back%"
        AND NOT EXISTS (SELECT 1 FROM ' . _DB_PREFIX_ . 'card_identity ci
                         WHERE ci.id_product = i.id_product)'
) ?: [];
foreach ($strays as $stray) {
    // Image::delete() clears the generated thumbnails and the join rows too.
    (new Image((int) $stray['id_image']))->delete();
}

line("stock back images added: $backsAdded, replaced: $backsReplaced, removed from non-cards: " . count($strays));
if (realBackPath() === null) {
...
// ---------------------------------------------------------------------------
/**
 * Ranked by the dearest SKU each product holds, not by its base price.
 *
 * Same reason the policy rule above changed: a slab's value lives in the
 * combination, and a sealed box has no combination at all. Ordering on the base
 * price put the sample entirely among raw singles, so the two kinds the copy
 * system had just been extended to cover were the two it never demonstrated.
 */
$sample = $db->executeS(
    'SELECT cc.id_product, MAX(p.price + IFNULL(pa.price, 0)) AS sku_price
       FROM ' . _DB_PREFIX_ . 'card_copy cc
       JOIN ' . _DB_PREFIX_ . 'product p ON p.id_product = cc.id_product
       LEFT JOIN ' . _DB_PREFIX_ . 'product_attribute pa
            ON pa.id_product_attribute = cc.id_product_attribute
      GROUP BY cc.id_product
 ...
 *
 * Judged on what the COPY sells for - the product price plus its combination's
 * impact - not on the product's base price. Those are the same number only for
 * raw near-mint singles. A slab is the case that breaks it: a PSA 10 of a $3
 * Japanese common is a $70 item sitting on a $3 product, and reading the base
 * price alone filed it as bulk and told the storefront it would never be
 * photographed. Sealed sits at id_product_attribute 0 with no combination at
 * all, so its impact is simply zero.
 *
 * The threshold is a seeding convenience. In production this is set at intake.
 */
$sku = 'SELECT COUNT(*) FROM ' . _DB_PREFIX_ . 'card_copy cc
          JOIN ' . _DB_PREFIX_ . 'product p ON p.id_product = cc.id_product
          LEFT JOIN ' . _DB_PREFIX_ . 'product_attribute pa
               ON pa.id_product_attribute = cc.id_product_attribute
         WHERE (p.price + IFNULL(pa.price, 0)) < ' . BULK_PRICE_CEILING . '
           AND cc.photo_state = "pending"';
$bulk = (int) $db->getValue($sku);
$db->execute(
...
/**
 * Anything at or above the ceiling is put BACK to per_copy.
 *
 * Without this the pass is one-way: a copy filed as bulk under the old base-price
 * rule would stay stock_only for ever, because the corrected rule only ever adds
 * to the set. Re-running has to be able to fix its own earlier verdict.
 */
$db->execute(
    'UPDATE ' . _DB_PREFIX_ . 'card_copy cc
       JOIN ' . _DB_PREFIX_ . 'product p ON p.id_product = cc.id_product
