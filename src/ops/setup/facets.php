...
$templates = [
    /**
     * The game root lists EVERYTHING - both regions, all three forms - so its
     * rail carries the axes that cut across them and none of the card-only
     * detail. This category alone: its children belong to the Singles and
     * Sealed templates below, and a descendants sweep here would steal them.
     */
    'DoubleSleeve - Pokémon' => [
        'path' => ['Pokémon'],
        'descendants' => false,
        'filters' => [
            ['price', SLIDER],
    ...
    'DoubleSleeve - Sealed' => [
        ...
        'filters' => [
            ...
            ['feat:Set', CHECKBOX],
            /**
             * Region is a FACET here and a category under Singles, and that
             ...
             *
             * A second "Print Region" feature used to sit directly below this one
             * - same question by a near-identical name, and every row of it the
             * same hardcoded literal. Retired; see migrations/retire-print-region.php.
             */
            ['feat:Region', CHECKBOX],
            /**
             * Language as a FEATURE here, where Singles has it as an attribute
             * group - sealed has no combinations to hang an axis on, and an
             * Elite Trainer Box is English or Japanese, never both at once.
             * Same label either way, so the shopper reads one question.
             *
             * It sits below Region because they answer different things: Region
             * is which catalogue a product comes out of - different sets,
             * different numbering, different schedule - and a Western catalogue
             * can be printed in several languages.
             *
             * Deliberately NOT added to the combined Pokémon template: that one
             * already carries `ag:Card Language` for singles, and a second rail
             * under the same name would read as two different questions.
             */
            ['feat:Card Language', CHECKBOX],
...
foreach ($templates as $name => $spec) {
    $categoryIds = ($spec['descendants'] ?? true)
        ? withDescendants(categoryIdByPath($spec['path']))
        : array_filter([categoryIdByPath($spec['path'])]);
