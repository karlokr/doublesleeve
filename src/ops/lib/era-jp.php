...
function resolveEraJp(string $name, string $abbreviation, string $publishedOn): string
{
    ...
    /**
     * Undated promo runs belong to no block.
     *
     * TCGplayer carries the 1990s Japanese promo lines - vending sheets, CoroCoro
     * inserts, Champion Road, the contest cards - with NO set code and no real
     * release date, and tcgcsv stamps those with the day of the export. That put
     * eighteen 1996-era promo sets into MEGA, the block whose window is "2026 and
     * later", purely because the export ran today. A set with no code that names
     * itself a promo is a promo, whatever date came back with it.
     */
    if ($abbr === '' && preg_match(
        '/promo|vending|contest|fan club|world hobby|champion road|battle road|gym challenge'
        . '|elementary school|how i became|master battle set|unnumbered|world championships'
        . '|information|quiz|campaign|magazine|prize|trophy/i',
        $name
    )) {
