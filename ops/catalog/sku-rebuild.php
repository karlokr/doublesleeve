...
foreach ($rows as $row) {
     ...
     *
     * The subtype is renamed for OUR catalogue first - see lib/printing.php.
     * TCGplayer names a printing only well enough to be unambiguous inside its
     * own group, and shadowed Base Set arrives as bare "Holofoil" because that
     * group holds nothing else. Keyed by the name we end up using, so the
     * combination is built against the attribute value that will be shown.
     */
    $groupId = (int) $row['tcgplayer_group_id'];
    $printings = [];
    foreach ($available as $subtype => $usd) {
        $name = printingName($groupId, (string) $subtype);
        if (isset($attributes['Printing'][$name])) {
            $printings[$name] = $usd * $usdCad;
        }
    }
    if (!$printings) {
        continue;
