...
$copy = [
    'en' => [
        ...
        'intro' => 'Every Pokémon TCG expansion, Western and Japanese, grouped by era. Looking for one card? '
    ...
    'fr' => [
        ...
        'intro' => 'Toutes les extensions du JCC Pokémon, occidentales et japonaises, regroupées par ère. Vous cherchez une carte précise ? '
...
foreach ($languages as $language) {
    ...
    /**
     * Singles > REGION > era > set.
     *
     * This walked Singles' children as eras, which was right until print region
     * was inserted between them: the page then rendered the two regions as era
     * headings and the real eras as "sets", so every tile fell back to a
     * typographic plate and 450 set logos vanished from the directory.
     */
    $regions = childCategories($singlesId, $idLang);
    ...
    // Jump nav, so the page is usable without scrolling through every block.
    $html .= '<div class="cc-sets__jump"><span class="cc-sets__jump-label">'
        . $e($strings['jump']) . '</span>';
    foreach ($regions as $region) {
        foreach (childCategories((int) $region['id_category'], $idLang) as $era) {
            $html .= '<a class="cc-sets__jump-link" href="#era-' . (int) $era['id_category'] . '">'
                . $e($region['name']) . ' &middot; ' . $e($era['name']) . '</a>';
        }
    }
    $html .= '</div>';

    foreach ($regions as $region) {
        $regionId = (int) $region['id_category'];
        $eras = childCategories($regionId, $idLang);
        $regionUrl = $link->getCategoryLink($regionId, $region['link_rewrite'], $idLang);

        $html .= '<div class="cc-sets__region" id="region-' . $regionId . '">'
            . '<h2 class="cc-sets__region-title"><a href="' . $e($regionUrl) . '">'
            . $e($region['name']) . '</a></h2>';
        ...
        foreach ($eras as $era) {
            ...
            $html .= '<div class="cc-sets__era-head">'
                . '<h3 class="cc-sets__era-title"><a href="' . $e($eraUrl) . '">' . $e($era['name']) . '</a></h3>'
            ...
            foreach ($sets as $set) {
                ...
                // Every tile is the same size. Sets without a logo get a
                // typographic plate built from the TCGplayer abbreviation, not a
                // shrunken card or an empty box - a ragged grid was the single
                // worst thing about the old list.
                $art = $hasLogo
