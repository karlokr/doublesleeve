...
class Cryptocards_copies extends Module
{
    ...
    public function hookActionCartSave(array $params): void
    {
        ...
        /**
         * Several serials, comma-separated, and the list REPLACES what was there.
         *
         * A shopper can pick more than one photographed copy of the same card, so
         * the field carries a list. It has to replace rather than merge, or
         * deselecting a copy would leave its serial recorded and the line would
         * keep asking for a card nobody wanted. An empty field therefore clears
         * the choices for the SKUs it names.
         */
        $chosen = array_values(array_filter(array_map(
            'trim',
            explode(',', (string) Tools::getValue('cc_copy_uid', ''))
        )));
        if ($chosen !== []) {
            $this->rememberChoices($cartId, $chosen);
        }
    ...
    /**
     * Store the shopper's chosen serials for this cart, without reserving them.
     *
     * @param array<int, string> $copyUids
     */
    private function rememberChoices(int $cartId, array $copyUids): void
    {
        $db = Db::getInstance();

        /**
         * Grouped by SKU first, because the replace is per LINE.
         *
         * Clearing by cart alone would wipe the choices made on every other line
         * the moment one line was edited.
         */
        $bySku = [];
        foreach ($copyUids as $copyUid) {
            $copy = $db->getRow(
                'SELECT id_product, id_product_attribute FROM ' . _DB_PREFIX_ . 'card_copy
                  WHERE copy_uid = "' . pSQL((string) $copyUid) . '"'
            );
            if (!$copy) {
                continue;
            }
            $bySku[(int) $copy['id_product'] . ':' . (int) $copy['id_product_attribute']][] = (string) $copyUid;
        }

        foreach ($bySku as $key => $uids) {
            [$productId, $skuId] = array_map('intval', explode(':', (string) $key));
    ...
    private function claimForCart(int $cartId): array
    {
        ...
        foreach ($lines as $line) {
            ...
            $claimed = $alreadyHeld + $this->reserve(
                ...
                $shortfall,
                $expiry,
                $this->chosenUids($cartId, $productId, $skuId)
            );
    ...
    /**
     * The serials this cart asked for on this line, oldest choice first.
     *
     * @return array<int, string>
     */
    private function chosenUids(int $cartId, int $productId, int $skuId): array
    {
        return array_column(Db::getInstance()->executeS(
            'SELECT copy_uid FROM ' . _DB_PREFIX_ . 'card_copy_choice
              WHERE id_cart = ' . $cartId . ' AND id_product = ' . $productId . '
                AND id_product_attribute = ' . $skuId . '
              ORDER BY date_add, copy_uid'
        ) ?: [], 'copy_uid');
    }

    /**
     * Reserves up to $count copies, honouring explicitly chosen serials first.
     *
    ...
    private function reserve(
        ...
        /**
         * Every chosen serial is attempted, and each is still only claimed if it
         * is genuinely available - the UPDATE is the lock. Stops early once the
         * line's quantity is satisfied, so choosing three copies and then
         * lowering the quantity to two reserves the first two rather than
         * over-reserving.
         */
        foreach ($chosenUids as $chosenUid) {
            if ($remaining <= 0) {
                break;
            }
            $db->execute(
                'UPDATE ' . _DB_PREFIX_ . 'card_copy
                  ...
                  WHERE copy_uid = "' . pSQL((string) $chosenUid) . '"
                    AND id_product = ' . $productId . ' AND id_product_attribute = ' . $skuId . '
